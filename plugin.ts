class DeclarationBundlerPlugin {
  ...
  constructor (options: any = {}) {
    ...
    if (!options.out) {
      throw new Error(`You need to set your output file in Plugin Options.\n\nExample:\nnew TypescriptDeclarationGenerator({out: './filename.d.ts'})`)
    }
    if (!options.moduleName) {
      throw new Error(`You need to set a module name in Plugin Options.\n\nExample:\nnew TypescriptDeclarationGenerator({module: 'your-module-name'})`)
    }
  ...
  private generateCombinedDeclaration (declarationFiles: Record<string, any>): string {
    var declarations = ''
